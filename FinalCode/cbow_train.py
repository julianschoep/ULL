...
if 'cbow_sents.pickle' not in os.listdir():
    ...
    with open('cbow_sents.pickle','wb') as f:
        pickle.dump(sentences, f)

        print(t(),'Saved {} sentences'.format(len(sentences)))
else:
    with open('cbow_sents.pickle','rb') as f:
        sentences = pickle.load(f)
if 'cbowmodel.pickle' not in os.listdir():
    model = gensim.models.Word2Vec(sentences, workers=4, min_count=1, size=100)
    with open('cbowmodel.pickle','wb') as f:
        pickle.dump(model,f)
else:
    with open('cbowmodel.pickle','rb') as f:
        model = pickle.load(f)

print(t(),'Writing learned vectors to file')
vec_file = open('cbow.vec','w')
