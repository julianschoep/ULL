...
if 'minskip_sents.pickle' not in os.listdir():
    ...
    with open('minskip_sents.pickle','wb') as f:
        pickle.dump(sentences, f)

        print(t(),'Saved {} sentences'.format(len(sentences)))
else:
    with open('minskip_sents.pickle','rb') as f:
        sentences = pickle.load(f)
if 'min_model.pickle' not in os.listdir():
    model = gensim.models.Word2Vec(sentences, sg=1, workers=4, min_count=1, size=100)
    with open('min_model.pickle','wb') as f:
        pickle.dump(model,f)
else:
...
vec_file = open('min_skipgram.vec','w')
